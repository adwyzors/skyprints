...
- [ ] `POST /auth/login` with valid credentials: 200, `ACCESS_TOKEN` + `REFRESH_TOKEN` cookies set
- [ ] `POST /auth/login` with wrong password: 401; `failedLoginAttempts` incremented in DB
- [ ] `POST /auth/login`, `User.isActive=false`: 401 "Account disabled. Contact administrator."
- [ ] `POST /auth/login`, `Login.isActive=false`: 401 "Login temporarily disabled. Contact administrator."
- [ ] `GET /auth/me`: same response shape as today — `{ id, alternateEmail, permissions[], user }`
- [ ] `POST /auth/refresh` with valid REFRESH_TOKEN: issues new ACCESS_TOKEN; `permissions[]` matches current `Login.permissions` in DB
- [ ] `POST /auth/refresh` after `revokeSession`: 401
- [ ] `POST /auth/refresh` after `softDelete`: 401 (`Login.isActive=false`)
- [ ] `POST /auth/logout`: increments `tokenVersion`; clears cookies; old REFRESH_TOKEN rejected on next refresh attempt
...
### Users module
- [ ] `GET /users` lists all non-deleted users; requires `users:view`
- [ ] `POST /users`: User + Login created in single transaction; `Login.permissions` defaults to `ROLE_PERMISSIONS[role]` when not supplied
- [ ] `POST /users` with custom permissions: `Login.permissions` matches the supplied array
- [ ] `POST /users` with duplicate active email: 409 ConflictException
- [ ] `POST /users` with soft-deleted user's email: 201 (partial unique index allows reuse)
- [ ] `POST /users` with non-existent or inactive `locationId`: 400 BadRequestException
- [ ] `PATCH /users/:id` role change: `Login.permissions` unchanged (role change does NOT reset permissions)
- [ ] `PATCH /users/:id/permissions`: permissions updated + `tokenVersion` incremented; user's next refresh returns new permissions in ACCESS_TOKEN
- [ ] `PATCH /users/:id/permissions` targeting self: 403
- [ ] `PATCH /users/:id/permissions` with unknown permission string: 400
- [ ] `PATCH /users/:id/permissions` with empty array `[]`: succeeds (user has no permissions)
- [ ] `POST /users/:id/revoke-session`: `tokenVersion` incremented; affected user's next refresh → 401 → redirected to /login
- [ ] `POST /users/:id/revoke-session` targeting self: 403
- [ ] `POST /users/:id/reset-password`: new `passwordHash` stored; `tokenVersion` incremented; affected user's old REFRESH_TOKEN rejected on next refresh
- [ ] `DELETE /users/:id`: `User.deletedAt` set + `Login.isActive=false` atomically; affected user cannot log in (401 generic)
- [ ] `GET /users/me`: returns `Login.permissions` from DB — not from ACCESS_TOKEN payload

### Frontend
- [ ] `NEXT_PUBLIC_INTERNAL_AUTH_ENABLED=false`: middleware redirects unauthenticated `/admin` and `/manager` to Keycloak URL (no change from today)
- [ ] `NEXT_PUBLIC_INTERNAL_AUTH_ENABLED=true`: middleware redirects to `/login?redirectTo=...`
- [ ] `/login` page renders email + password form
- [ ] Login with valid credentials: redirected to `redirectTo` param (or `/`)
- [ ] Login with wrong password: error message displayed, no redirect
- [ ] `/admin/users` page visible to ADMIN role; not accessible to MANAGER/OPERATOR (existing `RoleGuard` handles this)
- [ ] Create user modal: new User + Login appear in list after save
- [ ] Permissions drawer: "Load role defaults" pre-checks correct permissions for the selected role
- [ ] Revoke session button disabled on own row; shows confirmation dialog and succeeds on other rows
- [ ] Soft-delete removes user from list
- [ ] Password reset triggers tokenVersion increment (verify via API — the affected user's next refresh returns 401)
