...
export default function DeleteBillsModal({ isOpen, onClose, onSuccess }: Props) {
    ...
    const handleDelete = async () => {
        if (!startDate || !endDate || bills.length === 0 || !confirmed || (!taxEnabled && !challanEnabled)) return;

        setDeleting(true);
        try {
            const response = await deleteBillingContextsRange(startDate, endDate, taxEnabled, challanEnabled);
