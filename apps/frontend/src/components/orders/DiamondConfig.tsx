...
export default function DiamondConfig({
    ...
    const [editForm, setEditForm] = useState<DiamondRunValues | null>(null);
    ...
    const initialFormState: DiamondRunValues = {
        particulars: '',
        items: [],
        images: []
    };
    ...
    // --- Calculations ---
    const calculateRow = (item: DiamondItem): DiamondItem => {
    ...
    const updateItem = (index: number, field: keyof DiamondItem, value: any) => {
        setEditForm(prev => {
            if (!prev) return prev;
    ...
    const getTotals = (items: DiamondItem[]) => {
        ...
        const endRate = totalQty > 0 ? totalAmount / totalQty : 0;
        return { totalQty, totalAmount, endRate };
    };
    ...
    const saveRun = async (processId: string, runId: string) => {
        ...
        const apiValues = {
            ...
            // Summaries for display/reports
            'Total Quantity': totals.totalQty,
            'Total Amount': Number(totals.totalAmount.toFixed(2)),
            'End Rate': Number(totals.endRate.toFixed(2)),

            // Required for billing card display
    ...
    const renderRun = (process: any, run: ProcessRun) => {
        ...
        const data = mode === 'view'
            ? run.values as DiamondRunValues
                ...
                <div className="bg-white border border-gray-200 rounded p-3 sm:p-4 space-y-4">
                        ...
                        <div className="mb-3 flex items-start gap-2 p-2.5 bg-blue-50 border border-blue-100 rounded-lg">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-[10px] text-blue-400 mt-0.5 font-mono leading-relaxed truncate">
                                    Cols: <span className="text-blue-500">Design Sizes → Qty → F. Size → Diamond → Time → Rate</span>
                                </p>
                            </div>
                        </div>
                    )}
                        ...
                        <table className="w-full text-sm min-w-max">
                            <thead className="bg-gray-100 text-xs">
                                <tr>
                                    ...
                                    <th className="p-2 text-center">F. Size</th>
                                    <th className="p-2 text-left">Diamond</th>
                                    <th className="p-2 text-right">Time</th>
                                    <th className="p-2 text-right bg-blue-50">Rate</th>
                                    <th className="p-2 text-right bg-blue-50">Amount</th>
                            ...
                            <tbody>
                                {parseItems(data.items as DiamondItem[]).map((item, idx) => (
                                    <tr key={idx} className="border-t">
                                        <td className="p-2">
                                            {mode === 'edit' ? (
                                                <input className="w-full border p-1" value={item.designSizes} onChange={e => updateItem(idx, 'designSizes', e.target.value)} />
                                            ) : item.designSizes}
                                        </td>
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-16 border p-1 text-right" value={item.quantity} onChange={e => updateItem(idx, 'quantity', parseFloat(e.target.value) || 0)} />
                                            ) : item.quantity}
                                        </td>
                                        <td className="p-2 text-center text-gray-500">{item.fSize ?? 'all'}</td>
                                        <td className="p-2">
                                            {mode === 'edit' ? (
                                                <input className="w-full border p-1" value={item.diamond} onChange={e => updateItem(idx, 'diamond', e.target.value)} />
                                            ) : item.diamond}
                                        </td>
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-16 border p-1 text-right" value={item.time} onChange={e => updateItem(idx, 'time', parseFloat(e.target.value) || 0)} />
                                            ) : item.time}
                                        </td>
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-20 border p-1 text-right" value={item.rate} onChange={e => updateItem(idx, 'rate', parseFloat(e.target.value) || 0)} />
                                            ) : item.rate}
                                        </td>
                                        <td className="p-2 text-right bg-gray-50 font-semibold">{item.amount.toFixed(2)}</td>
                            ...
                            <tfoot className="bg-gray-100 font-bold text-gray-700">
                                <tr>
                                    ...
                                    <td colSpan={3}></td>
                                    <td className="p-2 text-right text-xs text-gray-500">End Rate: {totals.endRate.toFixed(2)}</td>
                    ...
                    {mode === 'edit' && (
                        <div className="flex justify-end gap-2 mt-4 pt-4 border-t">
                            <button
                                onClick={() => { setOpenRunId(null); setEditingRunId(null); }}
