...
export default function LaserConfig({
    ...
    const saveRun = async (processId: string, runId: string) => {
        ...
        if (!editForm.particulars) {
            toast.error('Please fill field Particulars');
            return;
        }
    ...
    const renderRunFormOrView = (process: any, run: ProcessRun) => {
        ...
        const handlePasteToFill = (e: React.ClipboardEvent<HTMLDivElement>) => {
            ...
            const newItems: LaserItem[] = rows.map(row => {
                const cols = row.split('\t');
                const qty = parseFloat(cols[1]?.replace(/,/g, '')) || 0;
                const laserTime = parseFloat(cols[3]?.replace(/,/g, '')) || 0;
                const rate = laserTime * 10;
                const amount = qty * rate;
                return {
                    designSizes: cols[0]?.trim() || '',
                    quantity: qty,
                    fSizes: cols[2]?.trim() || 'all',
                    laserTime: laserTime,
                    rate: rate,
                    amount: amount
                };
                        ...
                        <div className="mb-3 flex items-start gap-2 p-2.5 bg-blue-50 border border-blue-100 rounded-lg">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-[10px] text-blue-400 mt-0.5 font-mono leading-relaxed truncate">
                                    Cols: <span className="text-blue-500">Design Sizes → Qty → F. Size → Laser Time</span>
                                </p>
                            </div>
                        </div>
                    )}
