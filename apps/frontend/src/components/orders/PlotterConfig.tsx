...
export default function PlotterConfig({
            ...
            if (run) {
                ...
                setEditForm({
                    ...
                    items: existingItems.length > 0 ? existingItems : [{
                        fileSizes: '', quantity: 0, sizeW: 0, sizeH: 0,
                        layoutHeight: 0, layoutPcs: 0, sheetRate: 0,
                        sheetReq: 0, rate: 0, total: 0
                    }]
                });
    ...
    const renderRun = (process: any, run: ProcessRun) => {
        ...
        const data = mode === 'view'
            ? run.values as PlotterRunValues
                        ...
                        <div className="mb-3 flex items-start gap-2 p-2.5 bg-blue-50 border border-blue-100 rounded-lg">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-[10px] text-blue-400 mt-0.5 font-mono leading-relaxed truncate">
                                    Cols: <span className="text-blue-500">Files/Sizes → Qty → W → H → Layout H → Layout Pc → Sheet Rate</span>
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="overflow-x-auto border rounded">
                        <table className="w-full text-sm min-w-max">
                            ...
                            <tbody>
                                {parseItems(data.items as PlotterItem[]).map((item, idx) => (
                                    <tr key={idx} className="border-t">
                                        <td className="p-2">
                                            {mode === 'edit' ? (
                                                <input className="w-full border p-1" value={item.fileSizes} onChange={e => updateItem(idx, 'fileSizes', e.target.value)} />
                                            ) : item.fileSizes}
                                        </td>
                                        ...
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-16 border p-1 text-right" value={item.sizeW} onChange={e => updateItem(idx, 'sizeW', parseFloat(e.target.value) || 0)} />
                                            ) : item.sizeW}
                                        </td>
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-16 border p-1 text-right" value={item.sizeH} onChange={e => updateItem(idx, 'sizeH', parseFloat(e.target.value) || 0)} />
                                            ) : item.sizeH}
                                        </td>
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-16 border p-1 text-right" value={item.layoutHeight} onChange={e => updateItem(idx, 'layoutHeight', parseFloat(e.target.value) || 0)} />
                                            ) : item.layoutHeight}
                                        </td>
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-16 border p-1 text-right" value={item.layoutPcs} onChange={e => updateItem(idx, 'layoutPcs', parseFloat(e.target.value) || 0)} />
                                            ) : item.layoutPcs}
                                        </td>
                                        <td className="p-2 text-right">
                                            {mode === 'edit' ? (
                                                <input type="number" className="w-16 border p-1 text-right" value={item.sheetRate} onChange={e => updateItem(idx, 'sheetRate', parseFloat(e.target.value) || 0)} />
                                            ) : item.sheetRate}
                                        </td>
                                        {/* READ ONLY CALCS */}
                                        ...
                                        <td className="p-2 text-right bg-gray-50 font-semibold">{item.total.toFixed(2)}</td>
                            ...
                            <tfoot className="bg-gray-100 font-bold text-gray-700">
                                ...
                                <tr>
                                    <td colSpan={7} className="p-2 text-right">In Meters:</td>
