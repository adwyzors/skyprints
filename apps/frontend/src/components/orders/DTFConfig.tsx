...
export default function DTFConfig({
    ...
    const calculateRow = (item: DTFItem, rate: number): DTFItem => {
        ...
        const numberOfLayouts = item.pcsPerLayout > 0 ? Math.ceil(quantityRequired / item.pcsPerLayout) : 0;
        // "Area(w*h)"
    ...
    const getTotals = (items: DTFItem[], form: DTFRunValues) => {
        const calculatedItems = items.map(i => calculateRow(i, form.rate || 0));
        ...
        const totalMeter = calculatedItems.reduce((sum, i) => sum + ((i.height || 0) * (i.numberOfLayouts || 0)), 0) / 39.38;
        ...
        const efficiency = layoutTotalAmount > 0 ? (100 - (((actualMeter - layoutTotalAmount) / layoutTotalAmount) * 100)) : 0;
    ...
    const addItem = () => {
        setEditForm(prev => prev ? {
            ...prev,
            items: [...prev.items, { particulars: '', height: 0, pcsPerLayout: 0, quantityActual: 0, adjustment: 0, fromOther: 0 }]
        } : prev);
    };
    ...
    const renderRun = (process: any, run: ProcessRun) => {
        ...
        const totals = getTotals(data.items || [], data);
            ...
            <div className="bg-gray-50 border border-gray-300 rounded p-3">
                <div className="mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-sm">{mode === 'edit' ? `Configure DTF Run ${run.runNumber}` : `DTF Run ${run.runNumber}`}</h3>
                    </div>
                ...
                <div className="bg-white border border-gray-200 rounded p-4 space-y-6">
                    ...
                    {mode === 'edit' && (
                        <div className="grid grid-cols-2 gap-4">
                            <SearchableManagerSelect label="Executor" users={managers} valueId={runManagers[run.id]?.executorId ?? run.executor?.id} onChange={(id: string) => handleManagerSelect(run.id, 'executorId', id)} />
                            <SearchableManagerSelect label="Reviewer" users={managers} valueId={runManagers[run.id]?.reviewerId ?? run.reviewer?.id} onChange={(id: string) => handleManagerSelect(run.id, 'reviewerId', id)} />
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                        <div className="lg:col-span-1">
                            <label className="text-xs font-semibold text-gray-700 block mb-1">Particulars <span className="text-red-500">*</span></label>
                            {mode === 'edit' ? (
                                <input className="w-full border p-1 rounded text-sm" value={data.particulars || ''} onChange={e => updateField('particulars', e.target.value)} />
                            ) : <div className="text-sm border-b pb-1">{data.particulars}</div>}
                        </div>
                        <div>
                            <label className="text-xs font-semibold text-gray-700 block mb-1">
                                PCS <span className="text-red-500">*</span>
                                <span className="text-gray-500 text-[10px] ml-1">(Ord. Qty: {localOrder.quantity})</span>
                            </label>
                            {mode === 'edit' ? (
                                <input type="number" className="w-full border p-1 rounded text-sm" value={data.pcs} onChange={e => updateField('pcs', parseFloat(e.target.value) || 0)} />
                        ...
                        <div className="flex gap-4">
                            <label className="flex items-center gap-2 text-sm cursor-pointer border p-2 rounded hover:bg-gray-50 w-full justify-center">
                                <input type="checkbox" checked={data.isFusing} disabled={mode === 'view'} onChange={e => {
                            ...
                            {data.isFusing && (
                                <label className="flex items-center gap-2 text-sm cursor-pointer border p-2 rounded hover:bg-gray-50 w-full justify-center">
                                    <input type="checkbox" checked={data.isJobDifference} disabled={mode === 'view'} onChange={e => updateField('isJobDifference', e.target.checked)} className="w-4 h-4" />
                        ...
                        {data.isFusing && data.isJobDifference && (
                            <div>
                                ...
                                {mode === 'edit' ? (
                                    <input type="number" className="w-full border border-blue-300 bg-blue-50 p-1 rounded text-sm" value={data.customPcs} onChange={e => updateField('customPcs', parseFloat(e.target.value) || 0)} />
                    ...
                    <div className="border rounded overflow-hidden">
                        <button onClick={() => setExpandedLayouts(!expandedLayouts)} className="w-full bg-gray-100 p-2 flex items-center justify-between hover:bg-gray-200 transition-colors">
                            <span className="font-semibold text-sm text-gray-700 flex items-center gap-2"><Eye className="w-4 h-4" /> Layout Details</span>
                            {expandedLayouts ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                        </button>
                            ...
                            <div className="p-3 bg-white overflow-x-auto">
                                <table className="w-full text-xs">
                                    ...
                                    <tbody>
                                        {totals.items.map((item, idx) => (
                                            <tr key={idx} className="border-b last:border-0 hover:bg-gray-50">
                                                ...
                                                <td className="p-2">{mode === 'edit' ? <input type="text" className="w-full border rounded p-1 text-left" value={item.particulars || ''} onChange={e => updateItem(idx, 'particulars', e.target.value)} /> : item.particulars}</td>
                                                <td className="p-2 text-center text-gray-400">23</td>
                                                <td className="p-2 text-center">{mode === 'edit' ? <input type="number" className="w-16 border rounded p-1 text-center" value={item.height} onChange={e => updateItem(idx, 'height', parseFloat(e.target.value) || 0)} /> : item.height}</td>
                                                <td className="p-2 text-center">{mode === 'edit' ? <input type="number" className="w-16 border rounded p-1 text-center" value={item.pcsPerLayout} onChange={e => updateItem(idx, 'pcsPerLayout', parseFloat(e.target.value) || 0)} /> : item.pcsPerLayout}</td>
                                                <td className="p-2 text-center">{mode === 'edit' ? <input type="number" className="w-16 border rounded p-1 text-center" value={item.quantityActual} onChange={e => updateItem(idx, 'quantityActual', parseFloat(e.target.value) || 0)} /> : item.quantityActual}</td>
                                                <td className="p-2 text-center">{mode === 'edit' ? <input type="number" className="w-16 border rounded p-1 text-center" value={item.adjustment} onChange={e => updateItem(idx, 'adjustment', parseFloat(e.target.value) || 0)} /> : item.adjustment}</td>
                                                <td className="p-2 text-center">{mode === 'edit' ? <input type="number" className="w-16 border rounded p-1 text-center" value={item.fromOther} onChange={e => updateItem(idx, 'fromOther', parseFloat(e.target.value) || 0)} /> : item.fromOther}</td>
                                                <td className="p-2 text-center font-medium text-gray-600">{Math.round(item.quantityRequired || 0)}</td>
                                                <td className="p-2 text-center font-medium text-gray-600">{item.numberOfLayouts}</td>
                                                <td className="p-2 text-center text-gray-500">{item.area}</td>
                                                <td className="p-2 text-right font-medium text-blue-600">{item.pricePerLayout?.toFixed(2)}</td>
                                                <td className="p-2 text-right font-bold text-blue-600">{item.rowTotal?.toFixed(2)}</td>
                                    ...
                                    <tfoot className="bg-gray-50 font-semibold text-gray-700">
                                        <tr>
                                            <td colSpan={9} className="p-2 text-right text-gray-500 uppercase text-[10px] tracking-wider">Layout Totals</td>
                                ...
                                {mode === 'edit' && <button onClick={addItem} className="mt-2 text-xs flex items-center gap-1 text-blue-600 hover:text-blue-800"><Plus className="w-3 h-3" /> Add Layout Row</button>}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-gray-50 p-4 rounded border">
                        ...
                        <div className="text-right sm:text-left">
                            ...
                            <div className={`text-sm font-medium ${totals.efficiency < 0 ? 'text-red-500' : 'text-green-600'}`}>
                    ...
                    <div className="flex flex-col sm:flex-row justify-end items-end gap-6 bg-blue-50 p-4 rounded border border-blue-100">
                        {data.isFusing && (
                            <div className="text-right">
                                <label className="text-xs text-blue-400 uppercase tracking-wider block">Fusing Cost</label>
                                <div className="text-lg font-semibold text-blue-800">{totals.actualFusingCost.toFixed(2)}</div>
                        ...
                        < div className="text-right" >
                            <label className="text-xs text-gray-500 uppercase tracking-wider block">Per PC Cost</label>
                            <div className="text-lg font-medium text-gray-700">{totals.perPcCost.toFixed(2)}</div>
                        </div >
                        <div className="text-right">
                            <label className="text-xs text-gray-500 uppercase tracking-wider block">Actual Total</label>
                            <div className="text-2xl font-bold text-gray-900 border-b-2 border-gray-900 leading-none pb-1">{totals.actualTotal.toFixed(2)}</div>
                        </div>
                    </div >
                    ...
                    {process.runs.map(run => (
                        <div key={run.id} className="mb-4">
                            {!openRunId || openRunId !== run.id ? (
                                <div onClick={() => setOpenRunId(run.id)} className={`p-3 border rounded cursor-pointer flex justify-between items-center ${run.configStatus === 'COMPLETE' ? 'bg-green-50 border-green-200' : 'bg-white hover:bg-gray-50'}`}>
                                    <div className="flex items-center gap-2">
