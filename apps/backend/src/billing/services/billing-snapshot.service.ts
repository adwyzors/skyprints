...
export class BillingSnapshotService {
  ...
  private async adjustCustomerOutstanding(
    ...
    if (order?.customerId) {
      ...
      this.logger.log(
        `[OUTSTANDING] ${isAdd ? 'Added' : 'Subtracted'} ${decimalAmount} to customer ${order.customerId} for order ${orderId}`,
      );
    }
  }
  ...
  public async createGroupSnapshot(
    ...
    return this.createGroupSnapshotTx(billingContextId, {}, createdBy);
  }
  ...
  public async createGroupSnapshotTx(
    ...
    if (inputRequest && Object.keys(inputRequest).length) {
      ...
      await Promise.all(
        Object.entries(inputRequest).map(([orderId, orderInputs]) =>
          limit(() =>
            this.finalizeOrder(
              orderId,
    ...
    for (const snapshot of orderSnapshots) {
      for (const o of snapshot.billingContext.orders) {
        snapshotByOrderId.set(o.orderId, snapshot.inputs as OrderBillingInputs);
      }
    }
    ...
    for (const orderId of orderIds) {
      ...
      if (!orderInputs) {
        throw new Error(`Missing FINAL snapshot for order=${orderId}`);
      }

      if (!isOrderBillingInputs(orderInputs)) {
        throw new Error(`Invalid billing inputs for order=${orderId}`);
      }
    ...
    /* -------------------------------
           6️⃣ Calculate (NO TX)
        ------------------------------- */
    const calc = await this.calculator.calculateForGroupFromSnapshots(inputs);
    ...
    const snapshot = await this.prisma.transaction(async (tx) => {
      ...
      const tdsPercentage = tdsEnabled
      ...
      const s = await tx.billingSnapshot.create({
        data: {
          ...
          inputs: {
            ...Object.fromEntries(
              Object.entries(calc.perOrderCalculations).map(
                ([orderId, orderCalc]) => [
                  orderId,
                  {
                    ...orderCalc.inputs,
      ...
      if (s.version === 1) {
        for (const orderId of orderIds) {
          ...
          if (!order) {
            this.logger.error(
              `[finalizeGroup] Order ${orderId} not found during group finalization`,
            );
            continue;
          }
          ...
          if (!wasInOutstanding) {
            this.logger.warn(
              `[OUTSTANDING] Skipping balance reduction for order ${orderId} as it was never added (Status: ${order.statusCode})`,
            );
            continue;
          }
          ...
          if (!actualAmount.isZero()) {
            ...
            const currentBalance = new Prisma.Decimal(
              customer?.outstandingAmount?.toString() || '0',
            );
            ...
            this.logger.log(
              `[OUTSTANDING] Subtracted ${decrementAmount} from customer ${order.customerId} for order ${orderId} (Group Billed)`,
            );
          }
        }
    ...
    if (snapshot.version === 1) {
      ...
      for (const orderId of orderIds) {
        ...
        if (orderCalc) {
          let orderRevenue = 0;
          if (isTaxBill && grossGroupRevenue > 0) {
            const orderSubtotal = Number(orderCalc.result);
            orderRevenue =
              subtotalNum > 0
                ? orderSubtotal * (grossGroupRevenue / subtotalNum)
                : grossGroupRevenue / orderIds.length;
          }
  ...
  private async finalizeContextTx(
    ...
    this.logger.log(`[finalizeContextTx] Finalizing snapshot id=${draftId}`);
  ...
  async finalizeOrder(
    ...
    /* -------------------------------
        ...
        ------------------------------- */
    const context = await this.contextResolver.resolveOrderContext(
      this.prisma,
      orderId,
    );
    ...
    /* -------------------------------
           2️⃣ Calculate (NO TX)
        ------------------------------- */
    const calc = await this.calculator.calculateForOrder(orderId, inputs);
    ...
    const snapshot = await this.prisma.transaction(async (tx) => {
      ...
      const subtotal = calc.result;
      const taxPercentage = order.customer?.tax
        ? new Prisma.Decimal(5)
        : new Prisma.Decimal(0);
      const taxAmount = subtotal.mul(taxPercentage.div(100));
      ...
      // 🔑 HANDLE OUTSTANDING ADJUSTMENT (REDUCE ON INVOICE)
      this.logger.log(
        `[finalizeOrder] orderId=${orderId}, s.version=${s.version}, intent=${intent}`,
      );
      if (s.version === 1) {
        this.logger.log(`finalizeOrder order.statusCode=${order.statusCode}`);
        this.logger.log(
          `[OUTSTANDING] order.estimatedAmount=${order.estimatedAmount}, customerId=${order.customerId}`,
        );
        ...
        const oldEstimate = new Prisma.Decimal(
        ...
        if (wasAddedToOutstanding) {
          ...
          if (!adjustment.isZero()) {
            ...
            if (adjustment.isNegative()) {
              ...
              this.logger.log(
                `[OUTSTANDING] Decremented customer balance by ${decrementAmount} (Estimate: ${oldEstimate} -> Actual: ${newActual})`,
              );
            } else {
              await tx.customer.update({
                where: { id: order.customerId },
              ...
              this.logger.log(
                `[OUTSTANDING] Incremented customer balance by ${adjustment} (Estimate: ${oldEstimate} -> Actual: ${newActual})`,
              );
            }
          }
        } else {
          ...
          if (!newActual.isZero()) {
            ...
            this.logger.log(
              `[OUTSTANDING] Added full actual amount ${newActual} to customer balance`,
            );
          }
        }
  ...
  async finalizeGroup(
    ...
    return await this.createGroupSnapshotTx(
      billingContextId,
      inputs,
      createdBy,
    );
  ...
  /* =====================================================
       INTERNAL
    ===================================================== */
  private async transitionOrdersForContext(billingContextId: string) {}

  /* =====================================================
       PUBLIC — get latest snapshot
    ===================================================== */
  async getLatestSnapshot(dto: GetLatestBillingSnapshotDto) {
    let context;
    ...
    let tdsEnabled =
      (snapshot.inputs as any)?.__TDS_METADATA__?.tdsEnabled ?? false;
    let tdsPercentage =
      (snapshot.inputs as any)?.__TDS_METADATA__?.tdsPercentage || '0';
    let tdsAmount =
      (snapshot.inputs as any)?.__TDS_METADATA__?.tdsAmount || '0';
