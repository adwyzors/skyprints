...
export class BillingService {
  private readonly logger = new ContextLogger(BillingService.name);
  ...
  private validateDynamicInputs(inputs: Record<string, number>) {
    const variableRegex = /^[a-z][a-z0-9_]*$/;

    for (const [key, value] of Object.entries(inputs)) {
      if (!variableRegex.test(key)) {
        throw new BadRequestException(`Invalid variable name: ${key}`);
      }
  ...
  async calculateWithDynamicInputs(
    ...
    if (!formula) {
      throw new BadRequestException('Billing formula not configured');
    }
    ...
    for (const variable of requiredVars) {
      if (!(variable in staticVars) && !(variable in dynamicInputs)) {
    ...
    this.logger.log(`Merged variables: ${JSON.stringify(mergedVariables)}`);

    const compiledFormula = this.compiler.compile(formula);

    const result = this.engine.evaluate(compiledFormula, mergedVariables);

    this.logger.log(`Billing success run=${processRunId} amount=${result}`);
