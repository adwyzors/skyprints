...
export class CloudflareService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    if (
      ...
      throw new Error(
        'Missing Cloudflare R2 configuration. Please check your .env file.',
      );
    }
  ...
  private async compressImage(
    ...
    if (file.length <= targetSizeBytes) {
      this.logger.log(
        `Image already under ${targetSizeKB}KB, skipping compression`,
      );
      return file;
    }

    this.logger.log(
      `Compressing image from ${(file.length / 1024).toFixed(2)}KB to under ${targetSizeKB}KB`,
    );
  ...
  async deleteFileByUrl(fileUrl: string): Promise<void> {
    try {
      ...
      this.logger.error(`[R2][DELETE_FAILED] url=${fileUrl}`, error);
      throw new Error('File deletion failed');
    }
  }
  ...
  async deleteObject(key: string): Promise<void> {
    try {
      ...
      if (isNotFound) {
        this.logger.log(
          `[R2][DELETE_OBJECT_NOT_FOUND] key=${key} (treated as success)`,
        );
        return;
      }
