    ...
    for (const order of orders) {
      ...
      const sortedContexts = [...order.billingContexts].sort((a, b) => {
        if (order.statusCode === 'GROUP_BILLED') {
          if (
            a.billingContext.type === 'GROUP' &&
            b.billingContext.type !== 'GROUP'
          )
            return -1;
          if (
            b.billingContext.type === 'GROUP' &&
      ...
      const date =
        orderAnalytic?.billedAt || latestSnapshotDate || order.createdAt;
      ...
      for (const orderProcess of order.processes) {
        ...
        for (const run of orderProcess.runs) {
          ...
          if (preProductionLocationId || postProductionLocationId) {
            const preMatch =
              !preProductionLocationId ||
              run.preProductionLocationId === preProductionLocationId;
            const postMatch =
              !postProductionLocationId ||
          ...
          const isSublimation =
            processName.includes('all over sublimation') ||
            processName.includes('allover sublimation');

          if (isSublimation) {
            const mtrValue =
              runFields?.['Total Mtr'] || runBilling?.['total_mtr'];
          ...
          if (search) {
            ...
            const matchesOrderCode = order.code
              .toLowerCase()
              .includes(searchLower);
