...
export class AnalyticsService {
  ...
  async trackOrderFinalized(
    ...
    try {
      ...
      const cycleTimeHours =
        (finalizedDate.getTime() - creationDate.getTime()) / (1000 * 60 * 60);
      ...
      for (const orderProcess of order.processes) {
        ...
        for (const run of orderProcess.runs) {
          const contribution =
            revenuePerProcess / (orderProcess.runs.length || 1);
          const userWork = new Map<
            string,
            { isReviewer: boolean; isExecutor: boolean; user: any }
          >();

          if (run.reviewerId) {
            userWork.set(run.reviewerId, {
              isReviewer: true,
              isExecutor: false,
              user: run.reviewer,
            });
          ...
          if (effectiveLocId) {
            await this.prisma.locationAnalytics.upsert({
              ...
              update: {
                ...
                totalUnits: {
                  increment: order.quantity / (orderProcess.runs.length || 1),
                },
                totalRevenue: {
                  increment:
                    revenuePerProcess / (orderProcess.runs.length || 1),
      ...
      this.logger.error(
        `Error tracking finalized order ${orderId}: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async getWorkflowLifecycleMatrix(locationId?: string) {
    ...
    try {
      ...
      const orderIds = [
        ...new Set(activeRuns.map((r) => r.orderProcess.order.id)),
      ];
      ...
      snapshots.forEach((s) => {
        const valuePerOrder =
          Number(s.result) / (s.billingContext.orders.length || 1);
      ...
      filteredRuns.forEach((run) => {
        ...
        if (normalizedStatus === 'QC&COUNTING')
          normalizedStatus = 'QC & COUNTING';
  ...
  async getLiveProductionState() {
    try {
      ...
      this.logger.error(
        `Error fetching live production state: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async getActiveWorkload() {
    try {
      ...
      byReviewer.forEach((r) => {
        managerCountsMap.set(
          r.reviewerId!,
          (managerCountsMap.get(r.reviewerId!) || 0) + r._count._all,
        );
      });
      byExecutor.forEach((e) => {
        managerCountsMap.set(
          e.executorId!,
          (managerCountsMap.get(e.executorId!) || 0) + e._count._all,
        );
      });
