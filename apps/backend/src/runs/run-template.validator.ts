...
export class RunTemplateValidator {
  private readonly logger = new ContextLogger(RunTemplateValidator.name);
  ...
  async validateRunTemplates(
    ...
    if (templates.length !== ids.length) {
      this.logger.warn(`Invalid or inactive run template detected`);
      throw new BadRequestException('Invalid or inactive run template');
    }
